...
class XiaomiAirpurifierVa3 extends AirPurifierDevice {
  ...
  initDeviceProperties() {
    ...
    // Virtual Brightness
    this.addPropertyByString('screen:virtual-brightness', '{"siid":15,"piid":1,"type":"urn:miot-spec-v2:property:brightness:0000000D:xiaomi-va3:1","format":"float","access":["read","write","notify"],"unit":"percentage","description":"Virtual Brightness","valueRange":[0,100,1]}');
    const virtualBrightnessProp = this.getProperty('screen:virtual-brightness');
    virtualBrightnessProp.updateInternalValue(this.getVirtualBrightness());
    virtualBrightnessProp.getValue = this.getVirtualBrightness.bind(this);
    virtualBrightnessProp.setValue = this.setVirtualBrightness.bind(this);
    virtualBrightnessProp.getWriteProtocolObjForDid = this.virtualBrightnessGetWriteProtocolObjForDid.bind(this);
  }
  ...
  getVirtualBrightness() {
    const screenOn = this.getProperty('screen:on')?.getValue();
    const screenBrightness = this.getProperty('screen:brightness')?.getValue();
    if (!screenOn) return 0;
    if (screenBrightness === 0) return 50; // Dim brightness
    if (screenBrightness === 1) return 100; // Normal brightness
    return 100; // fallback
  }

  async setVirtualBrightness(value) {
    if (value <= 25) {
      this.setPropertyValue(this.getProperty('screen:on'), false);
      this.getProperty('screen:virtual-brightness').updateInternalValue(0);
    } else {
      if (value < 75) {
        this.setPropertyValue(this.getProperty('screen:brightness'), 0); // Dim
        this.getProperty('screen:virtual-brightness').updateInternalValue(50);
      } else {
        this.setPropertyValue(this.getProperty('screen:brightness'), 1); // Normal
        this.getProperty('screen:virtual-brightness').updateInternalValue(100);
      }
      this.setPropertyValue(this.getProperty('screen:on'), true);
    }
  }

  virtualBrightnessGetWriteProtocolObjForDid(deviceId, value) {
    this.setVirtualBrightness(value);
    return null;
  }
}
